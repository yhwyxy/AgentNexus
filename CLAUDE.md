...
curl -s localhost:8080/api/v1/mcp-servers        # {"items":[...],"count":N}
# full replacement of the mutable config; `revision` in the body is the optimistic lock (202 + Location)
curl -s -X PUT localhost:8080/api/v1/mcp-servers/<id> -d '{"revision":1,"displayName":"weather v2","runtime":{"type":"remote","remote":{"endpoint":"http://weather-mcp:8080/mcp"}}}'   # 409 on a stale revision
curl -s -X POST localhost:8080/api/v1/mcp-servers/<id>:refresh-tools   # 202; 409 if not enabled/running
curl -s -X POST localhost:8080/api/v1/mcp-servers/<id>:stop      # 200 + phase=stopped (synchronous recycle); tools vanish from the catalog
curl -s -X POST localhost:8080/api/v1/mcp-servers/<id>:start     # 202 (background rebuild); 409 while disabled
curl -s -X POST localhost:8080/api/v1/mcp-servers/<id>:restart   # 202; does not change revision
...
- `Validate()` only checks and never mutates. Defaulting (namespace `default`, timeouts 5s/10s/60s, maxInFlight 16, enabled true, desired state running) happens once, in `applyDefaults`/`applySpecDefaults` (`service.go`), the single site shared by `Register` and `Update`; `Register` then assigns the ID and timestamps, validates, and persists through the `Repository` interface. Validation failures wrap `ErrInvalid`; repository errors pass through unchanged. Rules: namespace/name match `^[a-z][a-z0-9-]{0,62}$` and are immutable after registration (rename = new server); remote endpoints are absolute http/https URLs with a host and no userinfo; process commands are absolute paths with args passed as `[]string` to `exec.CommandContext`, never through a shell; secrets go through `CredentialID`, never into headers, env, or URLs.
- `Repository` sentinel errors: `ErrNotFound`, `ErrAlreadyExists`, `ErrConflict` (optimistic-lock failure when `Update` is called with a stale `expectedRevision`; the adapter also maps "row does not exist" to `ErrConflict` on that path, because a deleted target and a stale revision are the same write failure), and `ErrNotRunnable` (the server is disabled or `desiredState=stopped`, so runtime work like a tool refresh is meaningless). `Server.Runnable()` is the single definition of that predicate; the SQLite adapter's `ListEnabled` is the repository-side listing the reconciler uses.
- `Revision` is the **config** version only. `Update` (full replacement of `UpdateInput`: displayName/description/labels/runtime/credential/timeouts/limits) bumps it inside one transaction; `SetEnabled` and `SetDesiredState` write the two runtime-intent fields **without** bumping it — otherwise every stop/start would manufacture `observedRevision != revision` drift and the reconciler would rebuild a deliberately stopped server. namespace/name/transport are immutable (`Update` never writes those columns), and zero-valued timeouts/limits fall back to the frozen defaults through `applyDefaults`, the single defaulting site shared by `Register` and `Update`.
...
- `NewHandler` builds the mux, wraps it in `withRequestLog` (outermost) and `withAuth`, and depends on consumer-defined interfaces (`ServerRegistry` with Register + Get + List + Update, `ServerRefresher`, `ServerLifecycle` with SetEnabled + Start + Stop + Restart, `Authenticator`, plus `AuditRecorder`), not on the concrete service. Tests inject the real service over a temp SQLite database, or a stub for failure paths; `Options.Auditor == nil` silently skips management-action audit events.
- Routes: `GET /health/live`, `GET /health/ready`, `GET /metrics` (admin only, registered from `Options.Metrics`), `GET /api/v1/mcp-servers` (collection envelope `{"items":[...],"count":N}`, order from the storage layer), `POST /api/v1/mcp-servers` (202 Accepted plus `Location`, because runtime start is asynchronous by design), `GET /api/v1/mcp-servers/{id}`, `PUT /api/v1/mcp-servers/{id}` (202 + `Location`; full replacement of the mutable config, optimistic lock via the required body `revision`), `POST /api/v1/mcp-servers/{id}:refresh-tools`, and the lifecycle actions `:enable`/`:disable`/`:start`/`:stop`/`:restart`.
- Action route mechanics: `net/http` requires a wildcard segment to own a whole path segment, so the route is `POST /api/v1/mcp-servers/{rest...}` and the handler splits `<id>:<action>` (`parseAction` in `mcp_server_handlers.go`). It is registered when *either* `Options.Refresher` or `Options.Lifecycle` is set; an action whose dependency is nil, and any unknown action name, return 404 `not_found` "unknown action" — never 405 and never a hint that the action exists but is unwired.
- Status codes follow observability, not taste: `:enable`/`:start`/`:restart` return 202 + `Location` (background instance build), `:disable`/`:stop` return 200 because they recycle synchronously and the response already carries `phase=stopped`. Lifecycle actions reject a non-empty body with 400 (silently ignoring body fields would make callers think they took effect). A non-runnable server (disabled or desired state stopped) → 409 `conflict` via `server.ErrNotRunnable`; a stale or missing `expectedRevision` → 409 `conflict` via `server.ErrConflict`.
